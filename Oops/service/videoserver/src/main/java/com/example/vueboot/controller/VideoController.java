...
import com.example.vueboot.server.MongoDbService;
...
public class VideoController {
    ...
    public R peVideo(@RequestParam Integer vid, HttpServletRequest request) {
        ...
        //获取mongodb的弹幕
//        Integer count=(Integer) redisTemplate.opsForHash().get("H_v");
...
//        String count = (String) redisTemplate.opsForHash().get("H_v", "7");
//        //String sy=(String) redisUtil.getObj("H_v");
//        System.out.println("视频喜欢数量" + count);
    ...
    //查询个人视频根据时间查询
    @RequestMapping("/nomday")
    public R nomday(@RequestParam Integer day, HttpServletRequest request) {
        System.out.println("天数"+day);
        String token = request.getHeader("token");
        DecodedJWT verify = JwtUtils.verify(token);
        Integer userid = verify.getClaim("userid").asInt();
        //获取jwt用户的id
        System.out.println("天数" + day + "用户id" + userid);
        List<Op_Video> videoList = videoServer.nomday(day, userid);
        System.out.println("查询的数据" + videoList);
        return  R.ok().data("video",videoList);
    }

    //删除个人的某个视频
    @PostMapping("/devideo")
    public R devideo(@RequestParam Integer vid, HttpServletRequest request) {
        System.out.println("视频id" + vid);
        String token = request.getHeader("token");
        DecodedJWT verify = JwtUtils.verify(token);
        Integer userid = verify.getClaim("userid").asInt();
        //获取jwt用户的id
        System.out.println("视频id" + vid + "用户id" + userid);
        int nums = videoServer.dvideo(vid, userid);
        if (nums > 0) {
