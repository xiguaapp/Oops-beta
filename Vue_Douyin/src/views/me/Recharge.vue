<template>
<div id="cls">
    <section class="aui-flexView">
	<header class="aui-navBar aui-navBar-fixed">
		<a href="/" class="aui-navBar-item">
			<i class="icon icon-return"></i>首页
		</a>
		<div class="aui-center">
			<span class="aui-center-title">充值中心</span>
		</div>
		<a href="chongzhijilu.html" class="aui-navBar-item">充值记录</a>
	</header>
  <section class="aui-scrollView">
	  <div class="aui-recharge-iphone">
            <div class="aui-well-bd">
				<p>&nbsp;Oops号</p>
                <p>{{Virtual}}</p>
            </div>
             <div class="aui-well-hd">
                 <img src="images/yinhangkahao.png" alt="">
             </div>
        </div>
		<div class="aui-blue-box">
			<div class="aui-palace aui-palace-one">
				<div class="aui-current-box">
					<a v-for="item in list" @click="cds(item.r,item.money)" href="javascript:;" class="aui-current-item " data-ydui-actionsheet="{target:'#actionSheet',closeElement:'#cancel'}">
				      <div class="aui-current-item-hd">
					<h1>{{item.r}}币</h1>
					<h3>售价:{{item.money}}元</h3>
				      </div>
			        </a>
				</div>	
			</div>
		</div>
	  <div class="aui-flex">
			<div class="aui-flex-box">
				<h1>消费记录</h1>
			</div>
		
		</div>
	  <div class="aui-job-list" v-for="item in consume">
		  <a href="javascript:;" class="aui-flex">
			  <div class="aui-flex-box">
				  <h2>{{item.type}}</h2>
                  <p>{{item.payTime}}</p>
              </div>
              <div class="aui-monthly">
				  <h3>-{{item.money}}</h3> 
              </div>
           </a> 
       </div>
	</section>
    <!-- <div class="m-actionsheet" id="actionSheet">
	<div class="aui-show-box">
		<div class="aui-show-box-title aui-footer-flex">
			<div class="aui-flex b-line">
				<i class="icon icon-close" id="cancel"></i>
				<div class="aui-flex-box">
					<h4>确认付款</h4>
				</div>
				<i class="icon icon-doubt"></i>
			</div>
			<div class="aui-coll-share-box">
				<h1><i>￥</i>25.00</h1>
			</div>
			<div class="aui-info-line">
				<a href="javascript:;" class="aui-flex b-line">
					<div class="aui-flex-box">
						<h5>订单信息</h5>
					</div>
					<div class="aui-text">转账</div>
				</a>
				<a href="javascript:;" class="aui-flex b-line">
					<div class="aui-flex-box">
						<h5>付款方式</h5>
					</div>
					<div class="aui-arrow">账户余额</div>
				</a>
			</div>
			<div class="aui-coll-cancel">
				<a href="javascript:;" id="cancel" class="">立即付款</a>
			</div>
		</div>
	</div>
</div> -->

<el-drawer
    :size="size"
  :visible.sync="drawer"
  :direction="direction"
  :before-close="handleClose">
  <span><div class="aui-flex b-line">
				<i class="icon icon-close" id="cancel"></i>
				<div class="aui">
					<h3>确认付款</h3>
				</div>
				<i class="icon icon-doubt"></i>
			</div>
            <div class="jage">
				<h1><i>￥</i>{{money.toFixed(2)}}</h1>&nbsp;
			</div>
            <div><span class="Rbi">{{r}}  R币</span></div>
            </br>
            <div class="aui-info-line">
				<a href="javascript:;" class="aui-flex b-line">
					<div class="aui-flex-box">
						<h5>订单信息</h5>
					</div>
					<div class="aui-text">充值</div>
				</a>
				<a href="javascript:;" class="aui-flex b-line">
					<div class="aui-flex-box">
						<h5>付款方式</h5>
					</div>
					<div class="aui-arrow">支付宝</div>
				</a>
			</div>
            <div class="aui-coll-cancel">
				<a href="javascript:;" id="cancel" class="">立即付款</a>
			</div>            
            </span>
</el-drawer>
</section>
</div>
</template>
<script>
//引入api
import { record } from '@/api/payment.js'

export default {
    
    data(){
        return{
            drawer: false,
            size: '57%',
            direction: 'btt',
            Virtual:'cxy123421',
            money:0.00,
            r:0,
            list:[{
                money:10,
                r:100
            },{
                money:50,
                r:500
            },{
                money:100,
                r:1000
            },{
                money:500,
                r:5000
            },{
                money:999,
                r:9999
            },{
                money:1200,
                r:12000
            }
            ],
            consume:[]
        }
   },
     methods: {
         cds(r,money){
             this.r=r
             this.money=money
         this.drawer=true;
         },
      handleClose(done) {
            done();
      }
    },
    created(){
           record().then(res => {
                console.log("充值记录"+res.data.message)
                this.consume=res.data.message;
                if (this.consume==null) {
                    console.log("暂无消费")
                }
           })
      }
}
</script>

<style scoped>


.Rbi{
    position: absolute;
    left: 47%;
    top: 31%;
}

.jage h1 {
    color: rgb(54, 54, 54);
    font-size: 2.5rem;
    font-weight: normal;
     position: absolute;
    left: 29%;
    top: 16%;
}

.aui{
    position: absolute;
    left: 41%;
    top: -137%;
    text-align: center;
}

.aui-flex-box h4 {
    color: rgb(5, 5, 5);
    font-weight: normal;
    font-size: 1rem;
}


* {
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
    box-sizing: border-box;
}

a {
    text-decoration: none;
    color: #000;
}

a, label, button, input, select {
    -webkit-tap-highlight-color: rgba(0, 0, 0, 0);
}

img {
    width: 100%;
    height: auto;
    display: block;
    border: 0;
}
/* .aui[data-v-0676fb98] {
    position: absolute;
    left: 41%;
    top: -137%;
    text-align: center;
}

.jage h1[data-v-0676fb98] {
    color: rgb(54, 54, 54);
    font-size: 2.5rem;
    font-weight: normal;
    position: absolute;
    left: 29%;
    top: 16%;
}
.aui-info-line[data-v-0676fb98] {
    margin-bottom: 5rem;
}
.aui-flex[data-v-0676fb98] {
    display: flex;
    align-items: center;
    padding: 10px;
    position: relative;
}
.aui-coll-cancel[data-v-0676fb98] {
    padding: 1rem;
}
.aui-coll-cancel a[data-v-0676fb98] {
    color: rgb(255, 255, 255);
    font-size: 0.9rem;
    width: 100%;
    height: 2.7rem;
    line-height: 2.7rem;
    display: block;
    background: rgb(16, 142, 233);
    border-radius: 3px;
    border-width: initial;
    border-style: none;
    border-color: initial;
    -o-border-image: initial;
    border-image: initial;
    text-align: center;
} */


body {
    background: #fff;
    color: #666;
}

html, body, div, dl, dt, dd, ol, ul, li, h1, h2, h3, h4, h5, h6, p, blockquote, pre, button, fieldset, form, input, legend, textarea, th, td {
    margin: 0;
    padding: 0;
}

a {
    text-decoration: none;
    color: #08acee;
}

button {
    outline: 0;
}

img {
    border: 0;
}

input{
	color:fff;
}

button,input,optgroup,select,textarea {
    margin: 0;
    font: inherit;
    color: inherit;
    outline: none;
}

li {
    list-style: none;
}

a {
    color: #666;
}

.r-line {
    position: relative;
}

.r-line:after {
    content: '';
    position: absolute;
    z-index: 0;
    top: 0;
    right: 0;
    height: 100%;
    border-right: 1px solid #D9D9D9;
    -webkit-transform: scaleX(0.5);
    transform: scaleX(0.5);
    -webkit-transform-origin: 100% 0;
    transform-origin: 100% 0;
}

.b-line {
    position: relative;
}

.b-line:after {
    content: '';
    position: absolute;
    z-index: 2;
    bottom: 0;
    left: 0;
    width: 100%;
    height: 1px;
    border-bottom: 1px solid #e2e2e2;
    -webkit-transform: scaleY(0.5);
    transform: scaleY(0.5);
    -webkit-transform-origin: 0 100%;
    transform-origin: 0 100%;
}

/* 必要布局样式css */
.aui-flexView {
    width: 100%;
    height: 100%;
    margin: 0 auto;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-orient: vertical;
    -webkit-box-direction: normal;
    -webkit-flex-direction: column;
    -ms-flex-direction: column;
    flex-direction: column;
}

.aui-navBar {
    height: 44px;
    position: relative;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    z-index: 102;
    background: #2955ff;
}

.aui-navBar-item {
    height: 44px;
    min-width: 25%;
    -webkit-box-flex: 0;
    -webkit-flex: 0 0 25%;
    -ms-flex: 0 0 25%;
    flex: 0 0 25%;
    padding: 0 0.9rem;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-align: center;
    -webkit-align-items: center;
    -ms-flex-align: center;
    align-items: center;
    font-size: 0.85rem;
    white-space: nowrap;
    overflow: hidden;
    color: #fff;
    position: relative;
}

.aui-navBar-item:first-child {
    -webkit-box-ordinal-group: 2;
    -webkit-order: 1;
    -ms-flex-order: 1;
    order: 1;
    margin-right: -25%;
}

.aui-navBar-item:last-child {
    -webkit-box-ordinal-group: 4;
    -webkit-order: 3;
    -ms-flex-order: 3;
    order: 3;
    -webkit-box-pack: end;
    -webkit-justify-content: flex-end;
    -ms-flex-pack: end;
    justify-content: flex-end;
}

.aui-center {
    -webkit-box-ordinal-group: 3;
    -webkit-order: 2;
    -ms-flex-order: 2;
    order: 2;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-pack: center;
    -webkit-justify-content: center;
    -ms-flex-pack: center;
    justify-content: center;
    -webkit-box-align: center;
    -webkit-align-items: center;
    -ms-flex-align: center;
    align-items: center;
    height: 44px;
    width: 50%;
    margin-left: 25%;
}

.aui-center-title {
    text-align: center;
    width: 100%;
    white-space: nowrap;
    overflow: hidden;
    display: block;
    text-overflow: ellipsis;
    font-size: 1.2rem;
    color: #fff;
}

.icon {
    width: 20px;
    height: 20px;
    display: block;
    border: none;
    float: left;
    background-size: 20px;
    background-repeat: no-repeat;
}

.icon-return {
    /* background-image:url('data:image/png;base64,'); */
}

.aui-scrollView {
    width: 100%;
    height: 100%;
    -webkit-box-flex: 1;
    -webkit-flex: 1;
    -ms-flex: 1;
    flex: 1;
    overflow-y: auto;
    overflow-x: hidden;
    -webkit-overflow-scrolling: touch;
    position: relative;
    padding-bottom:53px;
}

.aui-reset-phone{
    width:25px;
    height:25px;
}

.aui-recharge-iphone {
    display: -webkit-box;
    display: -webkit-flex;
    display: flex;
    -webkit-box-align: center;
    -webkit-align-items: center;
    align-items: center;
    padding: 15px;
    position: relative;
	background: #2955ff;
}

.aui-well-bd {
    -webkit-box-flex: 1;
    -webkit-flex: 1;
    flex: 1;
    min-width: 0;
    font-size: 14px;
    color: #fff;
}

.aui-well-hd {
    margin-left: .8em;
    width: 30px;
    height: 40px;
    line-height: 40px;
    text-align: center;
}

.aui-well-hd img {
    width: 100%;
    max-height: 100%;
    vertical-align: top;
    display: block;
    border: none;
}

.aui-well-bd p input {
    width: 100%;
    padding: 4px 0;
    font-size: 1.5rem;
    border: none;
    background: none;
	color: #FFF;
}

.aui-well-bd p {
    color: #a9c2fe;
    font-size: 0.8rem;
}

.aui-well-bd p em {
    font-style: normal;
}

.aui-recharge-iphone::after {
    content: '';
    position: absolute;
    z-index: 2;
    bottom: 0;
    left: 0;
    width: 100%;
    height: 1px;
    border-bottom: 0px solid #B2B2B2;
    -webkit-transform: scaleY(0.5);
    transform: scaleY(0.5);
    -webkit-transform-origin: 0 100%;
    transform-origin: 0 100%;
}


.aui-blue-box{
    width:100%;
    height:10rem;
    /* background-color:; */
    /* background-image: url("../images/beijing.jpg"); */
    background-size:cover;
    background-position:top center;
    position:relative;
    margin-bottom:4.5rem;
}

.aui-palace {
    padding: 0.5rem 0;
    position: relative;
    overflow: hidden;
}
.aui-palace-one{
    width:92%;
    position:absolute;
    left:50%;
    bottom:-2rem;
    margin-left:-46%;
    background:#fff;
    border-radius:5px;
    box-shadow:0 5px 15px #d2d0d0;
}

.aui-current-box {
    position: relative;
    background: #ffffff;
    padding: 20px 10px 1px 10px;
}

.aui-current-item {
    position: relative;
    float: left;
    padding: 10px 8px 10px 8px;
    width: 29.333333%;
    box-sizing: border-box;
    text-align: center;
    border-radius: 5px;
    border: 1px solid #108ee9;
    margin: 0 2% 4% 2%;
    height:4rem;
}

.aui-current-item-hd {
    width:auto;
    text-align: center;
    margin: 0 auto;
}

.aui-current-item-hd  h1{
    color:#108ee9;
    font-size:1.2rem;
}

.aui-current-item-hd  h3{
    color:#8e8e8e;
    font-size:0.1rem;
    font-weight:normal;
}

.m-actionsheet {
    text-align: center;
    position: fixed;
    bottom: 0;
    left: 0;
    width: 100%;
    z-index: 1000;
    background-color: rgba(255,255,255,0.9);
    -webkit-transform: translate(0, 100%);
    transform: translate(0, 100%);
    -webkit-transition: -webkit-transform .3s;
    transition: -webkit-transform .3s;
    transition: transform .3s;
    transition: transform .3s, -webkit-transform .3s;
}

.mask-black {
    background-color: rgba(0, 0, 0, 0.3);
    position: fixed;
    z-index: 500;
    bottom: 0;
    right: 0;
    left: 0;
    top: 0;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-pack: center;
    -webkit-justify-content: center;
    -ms-flex-pack: center;
    justify-content: center;
    -webkit-box-align: center;
    -webkit-align-items: center;
    -ms-flex-align: center;
    align-items: center;
    z-index: 998;
}

.actionsheet-action {
    display: block;
    margin-top: .15rem;
    font-size: 0.28rem;
    color: #555;
    height: 1rem;
    line-height: 1rem;
    background-color: #FFF;
}

.m-actionsheet {
    text-align: center;
    position: fixed;
    bottom: 0;
    left: 0;
    width: 100%;
    z-index: 10005;
    background-color: #ffffff;
    -webkit-transform: translate(0, 100%);
    transform: translate(0, 100%);
    -webkit-transition: -webkit-transform .3s;
    transition: -webkit-transform .3s;
    transition: transform .3s;
    transition: transform .3s, -webkit-transform .3s;
}

.actionsheet-toggle {
    -webkit-transform: translate(0, 0);
    transform: translate(0, 0);
}

.actionsheet-item {
    display: block;
    position: relative;
    font-size: 0.85rem;
    color: #555;
    height: 2rem;
    line-height: 2rem;
    background-color: #FFF;
}

.actionsheet-item {
    display: block;
    position: relative;
    font-size: 0.85rem;
    color: #555;
    height: 2rem;
    line-height: 2rem;
    background-color: #FFF;
}

.aui-coll-share-box {
    position: relative;
    overflow: hidden;
    padding: 1.8rem 0 1rem 0;
}

.aui-coll-share-item {
    position: relative;
    float: left;
    padding: 8px 10px;
    width: 33.333%;
    box-sizing: border-box;
    font-size: 12px;
    height: 85px;
}

.aui-coll-share-box h1 {
    color: #2f2f2f;
    font-size: 0rem;
    font-weight: normal;
}

.aui-coll-share-box h1 i {
    font-style: normal;
    color: #000000;
    font-size: 1.5rem;
}

.aui-flex {
    display: -webkit-box;
    display: -webkit-flex;
    display: flex;
    -webkit-box-align: center;
    -webkit-align-items: center;
    align-items: center;
    padding: 10px;
    position: relative;
}

.aui-flex-box {
    -webkit-box-flex: 1;
    -webkit-flex: 1;
    flex: 1;
    min-width: 0;
    font-size: 14px;
    color: #333;
}

.aui-flex-box h1 {
    color: #2f2f2f;
    font-size: 1.4rem;
    font-weight: normal;
}

.aui-flex-box h4 {
    color: #050505;
    font-weight: normal;
    font-size: 1rem;
}

.aui-flex-box h5 {
    text-align: left;
    color: #989898;
    font-weight: normal;
    font-size: 0.9rem;
}

.aui-flex-box h2{
    font-size:0.95rem;
    margin-bottom:0.3rem;
}

.aui-flex-box p{
    font-size:0.5rem;
    color:#b3b3b3;
}

.aui-monthly{
    text-align:right;
}

.aui-monthly h3{
    color:#2f2f2f;
    font-size:0.95rem;
}


.icon-close {
    /* background-image: url('data:image/png;base64,'); */
}

.aui-arrow {
    position: relative;
    padding-right: 0.8rem;
    color: #929292;
    font-size: 0.85rem;
}

.aui-arrow:after {
    content: " ";
    display: inline-block;
    height: 6px;
    width: 6px;
    border-width: 2px 2px 0 0;
    border-color: #848484;
    border-style: solid;
    -webkit-transform: matrix(0.71, 0.71, -0.71, 0.71, 0, 0);
    transform: matrix(0.71, 0.71, -0.71, 0.71, 0, 0);
    position: relative;
    top: -2px;
    position: absolute;
    top: 50%;
    margin-top: -4px;
    right: 2px;
    border-radius: 1px;
}

.aui-info-line {
    margin-bottom: 5rem;
}

.aui-coll-cancel a {
    color: red;

}

.aui-info-line .b-line:after {
    left: 15px;
}

.aui-text {
    color: #363636;
    font-size: 0.85rem;
}

.aui-coll-cancel {
    padding: 1rem;
}

.aui-coll-cancel a {
    color: rgb(255, 255, 255);
    font-size: 0.9rem;
    width: 100%;
    height: 2.7rem;
    line-height: 2.7rem;
    display: block;
    background: rgb(16, 142, 233);
    border-radius: 3px;
    border-width: initial;
    border-style: none;
    border-color: initial;
    border-image: initial;
        text-align: center;
}
</style>